...
class JsonDataLoader {
  static Future<Map<String, ShipJson>> loadJsonShipData(String gameDir, String? modDirName) async {
  ...
  static Future<Map<String, WeaponJson>> loadJsonWeaponData(String gameDir, String? modDirName) async {
    var gameDataPath = Directory(gameDir);
    // If modDirName is null, use vanilla
    var file =
        File(p.join(gameDataPath.path, modDirName?.let((it) => "mods/$it") ?? gameFilesPath(Directory(gameDir))?.path));
  ...
  static Map<String, Set<ShipSkin>> cacheShipSkins(List<ShipSkin> skins) {
  ...
  static Map<String, Set<String>> groupSkinHullIdsByHullId(List<ShipSkin> skins) {
  ...
  static Map<String, Set<String>> generateShipEnums(List<ShipJson> ships) {
  ...
  /// `load_stock_ship`
  static Future<List<ShipJson?>> loadShips(Directory folder) async {
  ...
  /// `load_stock_variant`
  static Future<List<Variant?>> loadVariants(Directory folder) async {
  ...
  /// `load_stock_skin`
  static Future<List<ShipSkin>> loadShipSkins(Directory folder) async {
  ...
  /// `load_stock_weapon`
  static Future<List<WeaponJson?>> loadWeapons(Directory folder) async {
  ...
  static Future<ShipJson?> loadShip(File file) async {
    return ShipJson.fromJson(parseRjson(await file.readAsString()) as Map<String, dynamic>);
  }

  static Future<Variant?> loadVariant(File file) async {
  ...
  static Future<ShipSkin?> loadShipSkin(File file) async {
    // var jsonEncodedYaml = json.encode(parseRjson(await file.readAsString()));
    // Fimber.v(jsonEncodedYaml);
    return ShipSkin.fromJson(parseRjson(await file.readAsString()) as Map<String, dynamic>);
  }

  static Future<WeaponJson?> loadWeapon(File file) async {
    // var jsonEncodedYaml = json.encode(parseRjson(await file.readAsString()));
    // Fimber.v(jsonEncodedYaml);
    return WeaponJson.fromJson(parseRjson(await file.readAsString()) as Map<String, dynamic>);
  }

  static Future<ModInfo?> loadModInfo(String gameDir, String modFolderName) async {
    var modInfoFile = File(p.join(gameDir, "mods", modFolderName, "mod_info.json"));
    try {
