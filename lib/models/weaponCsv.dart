...
part '../generated/models/weaponCsv.freezed.dart';

@unfreezed
class WeaponCsv with _$WeaponCsv {
  factory WeaponCsv({
    @Default("") String name,
    @Default("") String id,
    @Default("") String tier,
  ...
  static List<WeaponCsv> fromCsv(List<List<dynamic>> csvData) {
    var weapons = <WeaponCsv>[];
    var columnNames = csvData[0].map((e) => e.toLowerCase()).toList();
    for (var row in csvData.sublist(1)) {
      var weapon = WeaponCsv();
